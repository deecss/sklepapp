...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('product_description_test.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('product_test')
...
def main():
    ...
    try:
        ...
        logger.info(f"Checking if product exists in XML...")
        xml_product = product_manager.get_product_from_xml(product_id)
        if xml_product:
            logger.info(f"Product found in XML: {xml_product.get('name', 'No name')}")
            logger.info(f"Product URL: {xml_product.get('url', 'No URL')}")
        else:
